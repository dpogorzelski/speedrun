...
import (
	...
	portalpb "github.com/speedrunsh/portal-api/go/service"
	...
	for _, instance := range instances {
		...
		if useTunnel {
			...
			if ignoreFingerprint {
				grpcConn, err = portal.SSHTransportInsecure(instance.Address, k)
			} else {
				grpcConn, err = portal.SSHTransport(instance.Address, k)
			}
			if err != nil {
			...
			grpcConn, err = portal.HTTP2Transport(instance.Address)
			if err != nil {
				log.Errorf("%s:\n    %v\n", colors.Red(instance.Name), err)
				continue
			}
		...
		c := portalpb.NewPortalClient(grpcConn)

		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()

		r, err := c.ServiceRestart(ctx, &portalpb.Service{Name: serviceName})
		if err != nil {
